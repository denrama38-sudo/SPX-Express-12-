...
(function () {
  ...
  function killSplashForever() {
    ...
    // Hapus elemen biar tidak bisa dihidupkan lagi
  ...
  function guardLoop() {
    try {
      // Selalu pastikan splash mati setelah auth diputuskan
      if (authDecided) {
        killSplashForever();
      }
  ...
  function bootBridge() {
    ...
    // Kalau sudah aktivasi dari awal, langsung siapkan login
    if (isActivated()) {
      // Jangan bunuh splash terlalu cepat, biarkan skrip asli dulu
      setTimeout(function () {
        if (!authDecided) {
