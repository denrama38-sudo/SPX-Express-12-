/**
 * SPX Express 12 — Firebase Bridge (ANTI-LONCAT FINAL)
 * =====================================================
 * Versi ini memaksa Firebase jadi satu-satunya penguasa login gate.
 * Sistem Google Drive lama dinonaktifkan selama Firebase aktif.
 */
(function () {
  "use strict";
  ...
  var authDecided = false; // true setelah Firebase memberi tahu status login
  ...
  window.spxGoogleSignIn = function () {
    ...
    function onSuccess(user) {
      ...
      // Paksa hide berulang
      [50, 150, 300, 600, 1000, 2000].forEach(function (t) {
  ...
  window.spxGoogleSignOut = function () {
    ...
    function doOut() {
      // Langsung clear + tampilkan login (jangan tunggu Firebase)
      authDecided = true;
      clearLegacySession();
      ...
      var finish = function () {
        ...
        pr.then(function () {
          ...
          [100, 300, 600, 1200, 2000].forEach(function (t) {
  ...
  // ========== PENJAGA UTAMA (sangat agresif) ==========
  function guardLoop() {
    try {
      if (!authDecided) return; // tunggu Firebase dulu

      if (window.spxFirebase && window.spxFirebase.isLoggedIn && window.spxFirebase.isLoggedIn()) {
        // SUDAH LOGIN → pastikan overlay hilang
        hideGoogleLoginOverlay();
  ...
  // Nonaktifkan fungsi lama yang bisa bikin loncat
  function neutralizeLegacy() {
    try {
      // Override fungsi yang mungkin dipanggil kode Drive lama
      window.showGoogleLogin = function () {
        if (window.spxFirebase && window.spxFirebase.isLoggedIn && window.spxFirebase.isLoggedIn()) {
          hideGoogleLoginOverlay();
          return;
        }
        if (authDecided && isActivated()) showGoogleLoginOverlay();
      };
  ...
  function bootBridge() {
    // Jangan bunuh splash terlalu cepat
    // Biarkan Firebase dulu yang putuskan
    ...
    if (!window.spxFirebase) {
      ...
      // Kalau terlalu lama, paksa putuskan
      setTimeout(function () {
        if (!authDecided) {
          authDecided = true;
  ...
  // Backup: pastikan splash mati maksimal 8 detik
  setTimeout(function () {
    if (!authDecided) {
      authDecided = true;
      hideSplash();
