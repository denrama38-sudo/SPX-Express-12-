/**
 * SPX Express 12 — Firebase Bridge (ANTI-LONCAT v3)
 * =================================================
 * Fokus: hilangkan flash Home saat buka aplikasi.
 * Begitu app dibuka + sudah aktivasi → langsung kunci ke Google Login.
 * Splash ditahan sampai Firebase memutuskan status.
 */
(function () {
  "use strict";
  ...
  function keepSplash() {
    // Paksa splash tetap hidup (melawan skrip killSplash di index.html)
    var s = $("spxSplash");
    if (!s) return;
    s.classList.remove("spx-hide");
  ...
  function lockToLoginEarly() {
    // Dipanggil SEBELUM Firebase siap → cegah flash Home
    if (!isActivated()) return;
    if (authDecided && window.spxFirebase && window.spxFirebase.isLoggedIn && window.spxFirebase.isLoggedIn()) {
      return;
    }
    document.body.classList.add("spx-google-locked");
  ...
  window.spxGoogleSignIn = function () {
    ...
    function onSuccess(user) {
      ...
      [50, 150, 300, 600, 1200].forEach(function (t) {
        setTimeout(hideGoogleLoginOverlay, t);
      });
    ...
    function doOut() {
      ...
      var finish = function () {
        ...
        pr.then(function () {
          ...
          [100, 300, 700, 1500].forEach(function (t) {
            setTimeout(showGoogleLoginOverlay, t);
          });
        }).catch(function (e) {
  ...
  function guardLoop() {
    try {
      if (!authDecided) {
        // Belum putus → tetap kunci ke login (cegah flash Home)
        lockToLoginEarly();
        return;
      }
  ...
  function bootBridge() {
    // LANGSUNG kunci ke login kalau sudah aktivasi (sebelum Firebase siap)
    lockToLoginEarly();

    // Tahan splash selama mungkin
    keepSplash();
    setTimeout(keepSplash, 500);
    setTimeout(keepSplash, 1500);
    setTimeout(keepSplash, 3000);
    setTimeout(keepSplash, 4500);

    neutralizeLegacy();
    setInterval(guardLoop, 180);
    setInterval(neutralizeLegacy, 1000);
    ...
    window.spxFirebase.waitAuthReady().then(function (user) {
      ...
      if (user) {
        ...
        if (isActivated()) {
          showGoogleLoginOverlay();
          [100, 400, 900].forEach(function (t) {
            setTimeout(showGoogleLoginOverlay, t);
          });
        }
      }
    ...
    setTimeout(installSaveHook, 1200);
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", function () {
      // Langsung kunci biar Home tidak sempat muncul
      lockToLoginEarly();
      setTimeout(bootBridge, 200);
