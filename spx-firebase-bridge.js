/**
 * SPX Express 12 — Firebase Bridge (Phase 1A) — FIX LOGIN LOOP + ANTI LONCAT
 * ============================================================
 * Masalah yang diperbaiki:
 * - Setelah login Google, kode Drive lama cek KEY_G_TOKEN → kosong → login lagi
 * - Loncat-loncat Home ↔ Login overlay (race condition)
 * - Saat logout sempat muncul Home dulu
 * - Saat buka app belum login, Home sempat flash
 *
 * Perbaikan:
 * - Mirror sesi palsu ke KEY_G_USER agar kode lama diam
 * - Pakai popup (lebih stabil di WebView)
 * - Paksa hide/show overlay lebih agresif
 * - Interval penjaga lebih ketat
 * - Saat logout: clear dulu → baru show overlay (Home tidak sempat tampil)
 */
(function () {
  "use strict";
  ...
  function showGoogleLoginOverlay() {
    // Jangan tampilkan kalau Firebase sudah login
  ...
  window.spxGoogleSignIn = function () {
    ...
    function onSuccess(user) {
      ...
      // Paksa hide beberapa kali biar tidak loncat
  ...
  window.spxGoogleSignOut = function () {
    ...
    function doOut() {
      // 1. Langsung clear sesi lokal dulu (penting biar tidak loncat)
      clearLegacySession();
      updateFirebaseUI(null);

      // 2. Paksa tampilkan login overlay SEBELUM Firebase signOut selesai
      showGoogleLoginOverlay();

      var finish = function () {
        var pr = window.spxFirebase ? window.spxFirebase.signOut() : Promise.resolve();
        pr.then(function () {
          // Pastikan lagi setelah signOut
          clearLegacySession();
          updateFirebaseUI(null);
          showGoogleLoginOverlay();
          toast("Berhasil keluar");
          // Paksa beberapa kali biar tidak loncat balik ke Home
          setTimeout(showGoogleLoginOverlay, 100);
          setTimeout(showGoogleLoginOverlay, 300);
          setTimeout(showGoogleLoginOverlay, 600);
          setTimeout(showGoogleLoginOverlay, 1200);
        }).catch(function (e) {
  ...
  // Penjaga anti-loncat (lebih agresif)
    ...
    setInterval(function () {
      try {
        if (window.spxFirebase && window.spxFirebase.isLoggedIn && window.spxFirebase.isLoggedIn()) {
          ...
          // Belum login + sudah aktivasi → paksa tampil login
          if (isActivated()) {
            showGoogleLoginOverlay();
          }
        }
  ...
  function bootBridge() {
    ...
    if (!window.spxFirebase) {
      ...
      // Kalau Firebase belum siap tapi sudah aktivasi → tampilkan login dulu
      if (isActivated()) {
        showGoogleLoginOverlay();
      }
      return;
    ...
    window.spxFirebase.waitAuthReady().then(function (user) {
      ...
      if (user) {
        ...
        setTimeout(hideGoogleLoginOverlay, 400);
        setTimeout(function () { afterLogin(user); }, 500);
      } else {
        // Belum login → langsung clear sisa sesi + tampilkan overlay
        clearLegacySession();
        updateFirebaseUI(null);
        if (isActivated()) {
          showGoogleLoginOverlay();
          // Paksa beberapa kali biar tidak loncat
          setTimeout(showGoogleLoginOverlay, 200);
          setTimeout(showGoogleLoginOverlay, 600);
          setTimeout(showGoogleLoginOverlay, 1200);
        }
      }
  ...
  (function injectStyle() {
    var css = ".spx-fb-badge.on{background:#166534!important;color:#bbf7d0!important}" +
              "body.spx-google-locked #spxGoogleLogin{display:flex!important;visibility:visible!important;opacity:1!important;z-index:99999!important}";
